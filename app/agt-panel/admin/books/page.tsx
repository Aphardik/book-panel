...
export default function AdminBooksPage() {
    ...
    const someCurrentPageSelected =
        books.some((b) => selectedIds.has(b.id)) && !allCurrentPageSelected
    ...
    const confirmBulkDelete = async () => {
        ...
        try {
            ...
            // Using the bulkDelete api from api-client
            // Adjust this call to match your actual API client method:
            await booksApi.bulkDelete(ids)
            // Remove deleted books from local state
            setBooks((prev) => prev.filter((b) => !selectedIds.has(b.id)))
            setTotalItems((prev) => prev - selectedIds.size)
            toast({
                title: "Success",
                description: `${selectedIds.size} book(s) deleted successfully.`,
            })
            clearSelection()
        } catch (error) {
            ...
            toast({
                title: "Error",
                description: "Failed to delete selected books. Please try again.",
                variant: "destructive",
            })
        } finally {
        ...
        <div className="container mx-auto px-4">
            ...
            <Card className="shadow-sm border-muted">
                <CardHeader>
                    <div className="flex flex-col space-y-4">
                        ...
                        {/* ── Bulk Action Bar ── shows when books are selected */}
                        {selectedIds.size > 0 && (
                            <div className="flex items-center justify-between rounded-lg border border-destructive/30 bg-destructive/5 px-4 py-2.5 animate-in fade-in slide-in-from-top-1 duration-200">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-sm font-medium text-foreground">
                                        {selectedIds.size === 1 ? "1 book selected" : `${selectedIds.size} books selected`}
                                    </span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <Button
                            ...
                            <Table className="font-tiro">
                                    ...
                                    <TableRow className="bg-muted/50">
                                        ...
                                        <TableHead className="w-10 pl-4">
                                            <Checkbox
                                                ...
                                                ref={(el) => {
                                                    if (el) (el as any).indeterminate = someCurrentPageSelected
                                                }}
                                ...
                                <TableBody>
                                    {books.map((book) => (
                                        <TableRow
                                            key={book.id}
                                            className={`hover:bg-muted/30 transition-colors cursor-pointer ${selectedIds.has(book.id) ? "bg-primary/5 hover:bg-primary/10" : ""}`}
            ...
            <AlertDialog open={isBulkDeleteDialogOpen} onOpenChange={setIsBulkDeleteDialogOpen}>
                <AlertDialogContent className="rounded-2xl border-none shadow-2xl">
                    <AlertDialogHeader className="pb-4">
                        <AlertDialogTitle className="text-2xl font-bold text-destructive flex items-center gap-2">
                            <Trash2 className="h-6 w-6" /> Delete {selectedIds.size} Books
                        </AlertDialogTitle>
                        <AlertDialogDescription className="text-base text-muted-foreground">
                            You are about to permanently delete
                            <span className="font-bold text-foreground mx-1">{selectedIds.size} selected book(s)</span>
                            from the system. All associated data and images will be removed. This cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter className="gap-3">
                        <AlertDialogCancel className="h-12 rounded-xl border-2 font-bold px-6" disabled={bulkDeleting}>
                            Cancel
                        </AlertDialogCancel>
                        <AlertDialogAction
                            onClick={confirmBulkDelete}
                            disabled={bulkDeleting}
                            className="bg-destructive hover:bg-destructive/90 h-12 rounded-xl text-destructive-foreground font-bold px-8 shadow-lg shadow-destructive/20 border-none"
                        >
                            {bulkDeleting ? (
...
//     DialogTitle,
// } from "@/agt-panel/components/ui/dialog"
// import { booksApi, mastersApi, bulkDelete } from "@/agt-panel/lib/api-client"
// import ExcelImportDialog from "@/agt-panel/components/admin/excel-import-dialog"
// import { MultiSelect } from "@/agt-panel/components/ui/multi-select"
// import { FilterX, Filter, SlidersHorizontal } from "lucide-react"
// import {
